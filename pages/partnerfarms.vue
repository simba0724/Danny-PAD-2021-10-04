      ...
      <template slot="front">
        <v-card
          ...
          <div class="text-center">
            ...
            <span class="farm-rewards-caption">BJT Earned</span>
            <h5 class="stats-line">
              <img :src="farm_img">
              {{select}} Balance
            ...
            <h5 class="stats-line">
              <img :src="farm_img">
              BJT Price
              <span class="stats-line-info">{{round(bjtPrice*1000, 4)}}/K</span>
            </h5>
            <v-divider style="margin-left: 2%; width: 96%;"></v-divider>
          </div>
          <v-row
...
import { padAddress, BEP20ABI, partnerFarms } from '../farms_config.json'
export default {
  data () {
    return {
      approved: false,
  ...
  methods: {
    ...
    async loadNewInstances(select) {
      ...
      else this.contractABI = partnerFarms.defaultAbi
      this.tokenInstance = new this.dataseed.eth.Contract(this.tokenABI, this.tokenAddress)
      this.contractInstance = new this.dataseed.eth.Contract(this.contractABI, this.contractAddress)
    },
    ...
    async getDividends(){
      ...
      this.rewardsUsd = '$' + this.round(Number(amount*this.bjtPrice)/1e18, 4)
    },
    async  getPrice() {
    const busdInstance = new this.dataseed.eth.Contract(this.tokenABI, '0xe9e7cea3dedca5984780bafc599bd69add087d56')
    const bjtInstance = new this.dataseed.eth.Contract(this.tokenABI, '0x37f3ef4e065f3415d1b6f9284389cc9d9439f658')
    ...
    const pairBjtPadBal = await this.padInstance.methods.balanceOf('0x46c037bcda9706f8904cfb51255dc486a98d36ba').call()
    const pairBjtBalance = await bjtInstance.methods.balanceOf('0x46c037bcda9706f8904cfb51255dc486a98d36ba').call()
    this.padPrice = pairBusdBalance/pairPadBalance
    console.log(pairBjtPadBal, pairBjtBalance)
    this.bjtPrice = this.padPrice*(pairBjtPadBal/pairBjtBalance)
    },
    async getPoolInfo() {
      let amount = await this.contractInstance.methods.farmPool().call()
      this.pool_size = this.miOrK(Number(amount)/1e18) + ' BJT'
      this.pool_value = '$'+ this.miOrK(Number(amount*this.bjtPrice)/1e18)
    },
    async getBalance() {
    ...
    async getStakeVal(){
      let amount = await this.contractInstance.methods.sharesOf(this.myAddress).call()
      this.stake_value = '$' + this.round(Number(amount*this.lpPrice)/1e18, 6)
    },
    async getLpPrice() {
      ...
      console.log(reserve, 'testertester')
      this.lpPrice = this.round(reserve/(lpSupply/1e18), 10)
      console.log(lpSupply/1e18)
    },
    async getROI() {
      ...
      let roi = ((padBalance*this.bjtPrice*0.01)/(totalShares*this.lpPrice))*100
